...
export default function HomeScreen() {
  ...
  const loadDraftOrders = async () => {
    ...
    try {
      ...
      const response = await fetch('https://api.ttsa.uz/api/draft-orders/drafts', {
        method: 'GET',
        headers: {
  ...
  const handleEditDraft = async (draft: DraftOrder) => {
    try {
      ...
      const cartItemPromises = validProducts.map(async (product) => {
        ...
        const response = await fetch(`https://api.ttsa.uz/api/seller-mobile/products/${productId}`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json',
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const url = editingDraftId 
        ? `https://api.ttsa.uz/api/draft-orders/drafts/${editingDraftId}`
        : 'https://api.ttsa.uz/api/draft-orders/drafts';
  ...
  const handleDirectSale = async () => {
    ...
    if (customerData?.image && isBase64Image(customerData.image)) {
      ...
      console.log('No customer image or not Base64 format');
            ...
            console.log('URL:', 'https://api.ttsa.uz/api/order-history');
            ...
            const response = await fetch('https://api.ttsa.uz/api/order-history', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
          ...
          const response = await fetch('https://api.ttsa.uz/api/order-history/direct-order', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`,
    ...
    showConfirmation({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const response = await fetch(`https://api.ttsa.uz/api/draft-orders/drafts/${draftId}/confirm`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json',
    ...
    showConfirmation({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const response = await fetch(`https://api.ttsa.uz/api/draft-orders/drafts/${draftId}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${token}`,
              'Accept': 'application/json',
              ...
              <View style={styles.productInfo}>
                <View style={styles.productInfoHeader}>
                  {selectedProduct.image ? (
                    <Image source={{ uri: `https://api.ttsa.uz/uploads/products/${selectedProduct.image}` }} style={styles.selectedProductImage} />
                  ) : (
