...
const Categories = () => {
  ...
  const handleEditSuccess = () => {
    fetchCategories();
    setSnackbar({ open: true, message: 'Kategoriya muvaffaqiyatli yangilandi', type: 'success' });
  };

  const handleDeleteCategory = async () => {
    try {
      await api.delete(`/category/${selectedCategory.id}`, token);
      fetchCategories();
      setSnackbar({ open: true, message: "Kategoriya muvaffaqiyatli o'chirildi", type: 'success' });
  ...
  return (
    <div className="p-6">
                ...
                {user?.type === 'shop-owner' ? (
                  ...
                  <>
                    <button 
                      ...
                      className="inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-900 hover:bg-indigo-50 rounded-md transition-colors duration-200"
                    >
                      <PencilIcon className="h-4 w-4" /> Tahrirlash
                    </button>
                    <button 
                      onClick={() => { 
                        setSelectedCategory(category); 
      ...
      {isEditModalOpen && selectedCategory && (
        <EditCategoryModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} category={selectedCategory} onSuccess={handleEditSuccess} token={token} />
      )}
      {/* Delete Modal */}
      {isDeleteModalOpen && selectedCategory && (
        <DeleteCategoryModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} category={selectedCategory} onSubmit={handleDeleteCategory} />
      )}
      
      {/* View Modal for Shop Owners */}
