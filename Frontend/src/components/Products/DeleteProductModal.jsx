...
const DeleteProductModal = ({ isOpen, onClose, onSubmit, product, token: propToken }) => {
  ...
  const handleDelete = async () => {
    ...
    setError('');
    try {
      await api.delete(`/product/${product.id}`, token);
      onSubmit && onSubmit();
      onClose();
    } catch (err) {
          ...
          <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
            ...
            <div className="sm:flex sm:items-start">
              ...
              <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                <Dialog.Title as="h3" className="text-base font-semibold leading-6 text-gray-900">Maxsulotni o'chirish</Dialog.Title>
                <div className="mt-2">
                  <p className="text-sm text-gray-500">Siz rostdan ham {product?.name} maxsulotini o'chirmoqchimisiz?</p>
                  {error && <div className="mt-2 p-2 bg-red-100 border border-red-400 text-red-700 rounded">{error}</div>}
                </div>
              </div>
            </div>
            <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
              <button type="button" className="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto" onClick={handleDelete} disabled={loading}>{loading ? "O'chirilmoqda..." : "O'chirish"}</button>
