...
const EditProductModal = ({ isOpen, onClose, onSubmit, product, categories, subcategories, fetchSubcategories }) => {
  const { token } = useAuth();
  const [formData, setFormData] = useState({ id: '', name: '', price: '', category: '', subcategory: '', quantity: '', status: 'active' });
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.category?.id) fetchSubcategories(product.category.id);
    }
  }, [product, fetchSubcategories]);

  useEffect(() => {
    if (!isOpen) {
      setFormData({ id: '', name: '', price: '', category: '', subcategory: '', quantity: '', status: 'active' });
      setError('');
    }
  }, [isOpen]);
          ...
          <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
            ...
            <div className="sm:flex sm:items-start">
              <div className="mt-3 text-center sm:mt-0 sm:text-left w-full">
                <Dialog.Title as="h3" className="text-base font-semibold leading-6 text-gray-900">Maxsulotni o'zgartirish</Dialog.Title>
                <div className="mt-4">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                      <button type="submit" className="inline-flex w-full justify-center rounded-md bg-blue-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 sm:ml-3 sm:w-auto" disabled={loading}>{loading ? "Saqlanmoqda..." : "Saqlash"}</button>
