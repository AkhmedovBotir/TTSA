...
const EditAgentModal = ({ isOpen, onClose, agent, onSuccess, token }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!body.password) delete body.password;
              const response = await fetch(`https://api.ttsa.uz/api/agent/${agent.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Agentni tahrirlashda xatolik yuz berdi');
      }
          ...
          <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <PasswordInput name="password" value={formData.password} onChange={handleChange} className="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent sm:text-sm" />
              </div>
