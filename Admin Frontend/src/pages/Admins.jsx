...
const permissions = [
  { id: 'view_dashboard', label: 'Dashboard ko\'rish' },
  { id: 'manage_admins', label: 'Adminlar' },
  { id: 'manage_shop_owners', label: "Do'kon egalari" },
  { id: 'manage_stores', label: "Do'konlar" },
  { id: 'manage_sellers', label: 'Sotuvchilar' },
  { id: 'manage_regions', label: 'Regionlar' },
  ...
  { id: 'manage_notifications', label: 'Xabarnomalar' },
  { id: 'manage_settings', label: 'Sozlamalar' }
];
...
const Admins = () => {
  ...
  const handleViewAdmin = async (admin) => {
    ...
    try {
      const response = await fetch(`https://api.ttsa.uz/api/admin/${admin.id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
          'Accept': 'application/json',
  ...
  const handleEditSubmit = async (updatedData) => {
    if (!selectedAdmin?.id) {
      console.error('Admin ID topilmadi:', selectedAdmin);
      setSnackbar({
        open: true,
        message: 'Admin ma\'lumotlari to\'liq emas',
        type: 'error'
    ...
    try {
      const response = await fetch(`https://api.ttsa.uz/api/admin/${selectedAdmin.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleCreateAdmin = async (formData) => {
    try {
      const response = await fetch('https://api.ttsa.uz/api/admin/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
