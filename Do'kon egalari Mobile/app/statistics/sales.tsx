...
export default function SalesStatisticsScreen() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `https://api.ttsa.uz/api/shop-owner-mobile/statistics/sales?startDate=${format(dateRange.startDate, 'yyyy-MM-dd')}&endDate=${format(dateRange.endDate, 'yyyy-MM-dd')}`,
        {
          headers: {
  ...
  const safeStats = {
    overview: {
      ...
      totalAmount: parseNumber(stats?.overview?.totalAmount)
    },
    ...
    topSellingProducts: (stats?.topSellingProducts || []).map(product => ({
      _id: product?._id || '',
      productName: product?.productName || 'Noma\'lum mahsulot',
      totalQuantity: parseNumber(product?.totalQuantity),
      totalAmount: parseNumber(product?.totalAmount)
    }))
  };
