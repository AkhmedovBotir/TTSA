...
export default function WarehouseStatisticsScreen() {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(API_ENDPOINTS.STATISTICS.WAREHOUSE, {
        headers: {
    ...
    >
      ...
      <View style={styles.overviewContainer}>
        <View style={[styles.card, styles.overviewCard]}>
          ...
          <Text style={styles.cardValue}>{formatNumber(stats?.totalProducts || stats?.overview?.totalProducts)}</Text>
          <Text style={styles.cardLabel}>Mahsulotlar</Text>
        </View>
        <View style={[styles.card, styles.overviewCard]}>
          <Ionicons name="checkmark-circle-outline" size={32} color="#2D1B69" />
          <Text style={styles.cardValue}>{formatNumber(stats?.inStock || stats?.overview?.totalQuantity)}</Text>
          <Text style={styles.cardLabel}>Omborda</Text>
        </View>
        <View style={[styles.card, styles.overviewCard]}>
          <Ionicons name="cash-outline" size={32} color="#2D1B69" />
          <Text style={styles.cardValue}>{formatCurrency(stats?.totalValue || stats?.overview?.totalValue)}</Text>
      ...
      <View style={styles.section}>
        ...
        {stats?.categories ? Object.entries(stats.categories).map(([categoryName, categoryData]: [string, any]) => (
          <View key={categoryName} style={styles.categoryCard}>
            ...
            <View style={styles.categoryDetails}>
              <Text style={styles.categoryDetail}>
                Mahsulotlar: {formatNumber(categoryData.count)}
              </Text>
              <Text style={styles.categoryDetail}>
                Kam qolgan: {formatNumber(categoryData.lowStock || 0)}
              </Text>
            </View>
          </View>
      ...
      <View style={[styles.section, styles.lastSection]}>
        ...
        {stats?.lowStockProducts && stats.lowStockProducts.length > 0 ? (
          ...
          <View style={styles.emptyContainer}>
            <Ionicons name="checkmark-circle-outline" size={48} color="#34A853" />
            <Text style={styles.emptyText}>Barcha mahsulotlar yetarli miqdorda</Text>
          </View>
        )}
      </View>
