...
export default function CategoriesScreen() {
  ...
  const onRefresh = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_MOBILE_URL}/category/list?page=1&limit=${pagination.perPage}`, {
        headers: {
  ...
  const fetchCategories = async (page = 1) => {
    ...
    try {
      ...
      let url = `${API_MOBILE_URL}/category/list?page=${page}&limit=${pagination.perPage}`;
      if (searchQuery) {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      const response = await fetch(`${API_MOBILE_URL}/category/create`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleEditCategory = async () => {
    ...
    try {
      const response = await fetch(`${API_MOBILE_URL}/category/${editingCategory._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
    ...
    Alert.alert(
      ...
      [
        { text: 'Bekor qilish', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const response = await fetch(`${API_MOBILE_URL}/category/${categoryId}`, {
                method: 'DELETE',
                headers: {
                  'Authorization': `Bearer ${token}`,
                  'Content-Type': 'application/json',
