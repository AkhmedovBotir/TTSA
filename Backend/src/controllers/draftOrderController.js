...
const createDraftOrder = async (req, res) => {
    try {
        ...
        if (!storeOwner) {
            return res.status(400).json({
                success: false,
                message: 'Do\'kon egasi kiritilishi shart'
            });
        }
        ...
        const newDraftOrder = new DraftOrder({
            seller: req.user._id, // Use _id from the authenticated user
        ...
        if (req.user.role === 'agent') {
            ...
            // Find products that belong to seller's shop
            const sellerProducts = await Product.find({
                _id: { $in: productIds },
                shop: req.user.shopId || req.user.shop
            });
            
            console.log('Found seller products:', sellerProducts);
            
            if (sellerProducts.length === 0) {
                throw new Error('Sizning do\'koningizda bunday mahsulotlar topilmadi');
            }
...
const confirmDraftOrder = async (req, res) => {
    try {
        ...
        const { id } = req.params;
        let { paymentMethod = 'cash', customer, installmentDuration } = req.body;
        ...
        try {
            ...
            if (!resolvedShop && (req.user.shopId || req.user.shop)) {
                // Fallback to seller's shop
                resolvedShop = await Shop.findById(req.user.shopId || req.user.shop).populate('owner');
            }
        ...
        if (paymentMethod === 'installment') {
            try {
                ...
                if (!resolvedOwnerId) {
                    return res.status(400).json({ success: false, message: "Do'kon egasi topilmadi" });
                }
                ...
                const monthlyPayment = Math.ceil(totalSumNumber / installmentDuration);
