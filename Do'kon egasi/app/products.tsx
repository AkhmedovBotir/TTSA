...
interface FormState {
  ...
  unit: string;
  unitSize: string;
  quantity: string;  // Changed from quantity to quantity
...
const CreateScreen = ({
  ...
  const [form, setForm] = useState<FormState>({
    ...
    unit: 'dona',
    unitSize: '1',
    quantity: '0',  // Changed from quantity to quantity
    image: undefined
  });
...
export default function ProductsScreen() {
  ...
  // Create product function as suggested by backend developer
  const createProduct = async (productData: any) => {
    try {
      console.log('=== CREATE PRODUCT DEBUG ===');
      console.log('Input productData:', productData);
      
      // Check if image is base64 or file URI
      ...
      if (productData.image) {
        if (productData.image.startsWith('data:image')) {
          ...
          try {
            ...
            finalProductData.image = `data:${mimeType};base64,${base64}`;
      ...
      console.log('Final product data to send:', finalProductData);
  ...
  const handleEditProduct = async () => {
    ...
    try {
      ...
      formData.append('unit', productForm.unit || 'dona');
      formData.append('unitSize', unitSize.toString());
      formData.append('status', 'active');

      // Add image if selected
      if (productForm.image) {
        const imageUri = productForm.image;
        ...
        console.log('Adding image to FormData for edit:', {
          uri: imageUri,
          type: imageType,
          name: imageName,
        });
        ...
        console.log('No image selected for edit');
      }

      console.log('Updating product with data and image:', productForm.image ? 'Yes' : 'No');
