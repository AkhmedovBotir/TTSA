    ...
    try {
        ...
        try {
            ...
            if (decoded.role === 'admin' || decoded.role === 'general') {
                ...
                userType = 'agent';
            } else if (decoded.role === 'shop-owner') {
                console.log('Looking for shop owner with ID:', userId);
                user = await ShopOwner.findById(userId);
                console.log('Found shop owner:', user ? 'Yes' : 'No');
                userType = 'shop-owner';
            }
            ...
            // Adminlar va shop owner uchun faollik holatini tekshirish
            if ((userType === 'admin' || userType === 'general' || userType === 'shop-owner') && user.status !== 'active') {
