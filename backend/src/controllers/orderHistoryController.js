...
// Admin uchun barcha buyurtmalar tarixini olish (barcha turdagi buyurtmalar bilan)
const getAdminOrderHistory = async (req, res) => {
    try {
        const { page = 1, limit = 20, status, paymentMethod, startDate, endDate, orderType } = req.query;
        const skip = (parseInt(page) - 1) * parseInt(limit);
        ...
        console.log('Query for OrderHistory:', JSON.stringify(query, null, 2));
        const [orderHistory, installmentPayments, userOrders] = await Promise.all([
            // OrderHistory (seller orders)
            OrderHistory.find(query).sort({ createdAt: -1 }).lean(),
            // InstallmentPayment (installment orders)
            mongoose.model('InstallmentPayment').find(query).sort({ createdAt: -1 }).lean(),
            // Order (user mobile orders)
            mongoose.model('Order').find(query).sort({ createdAt: -1 }).lean()
        ]);

        console.log('OrderHistory found:', orderHistory.length);
        console.log('InstallmentPayments found:', installmentPayments.length);
        console.log('UserOrders found:', userOrders.length);

        // Combine all orders
        let allOrders = [
            ...orderHistory.map(order => ({ ...order, orderType: 'seller' })),
            ...installmentPayments.map(order => ({ ...order, orderType: 'installment' })),
            ...userOrders.map(order => ({ ...order, orderType: 'user' }))
        ];

        // Filter by orderType if specified
        if (orderType) {
            allOrders = allOrders.filter(order => order.orderType === orderType);
        }
        ...
        console.log('Paginated orders:', paginatedOrders.length);

        // Get unique IDs for population
        const sellerIds = [...new Set(paginatedOrders.map(order => order.seller?.toString()).filter(Boolean))];
        const storeOwnerIds = [...new Set(paginatedOrders.map(order => order.storeOwner?.toString()).filter(Boolean))];
        const userIds = [...new Set(paginatedOrders.map(order => order.user?.toString()).filter(Boolean))];

        // Fetch all related data in bulk
        const [agents, sellers, storeOwners, users] = await Promise.all([
            sellerIds.length > 0 ? mongoose.model('Agent').find({ _id: { $in: sellerIds } }, 'fullname phone').lean() : [],
            sellerIds.length > 0 ? mongoose.model('Seller').find({ _id: { $in: sellerIds } }, 'fullName username phone shop').lean() : [],
            storeOwnerIds.length > 0 ? mongoose.model('ShopOwner').find({ _id: { $in: storeOwnerIds } }, 'name phone').lean() : [],
            userIds.length > 0 ? mongoose.model('Client').find({ _id: { $in: userIds } }, 'fullName phone').lean() : []
        ]);
        ...
        const userMap = users.reduce((acc, user) => {
            acc[user._id.toString()] = { _id: user._id, name: user.fullName, phone: user.phone };
            return acc;
        }, {});

        // Populate orders with related information
        const populatedOrders = await Promise.all(paginatedOrders.map(async (order) => {
            let seller = null;
            let storeOwner = null;
            let user = null;
            ...
            if (order.seller) {
                ...
                if (!order.storeOwner && seller && seller.shop) {
                    try {
            ...
            // Handle storeOwner information
            if (order.storeOwner) {
                storeOwner = storeOwnerMap[order.storeOwner.toString()] || null;
            }

            // Handle user information (for user mobile orders)
            if (order.user) {
                user = userMap[order.user.toString()] || null;
            }

            // Format order based on type
            let formattedOrder = {
                ...order,
                seller,
                storeOwner,
            ...
            if (order.orderType === 'installment') {
                formattedOrder.installmentInfo = {
                    duration: order.installment?.duration,
                    interestRate: order.installment?.interestRate,
                    interestAmount: order.installment?.interestAmount,
                    totalWithInterest: order.installment?.totalWithInterest,
                    monthlyPayment: order.installment?.monthlyPayment
                };
            }
        ...
        res.status(200).json({
            ...
            summary: {
                totalOrders: total,
                sellerOrders: orderHistory.length,
                installmentOrders: installmentPayments.length,
