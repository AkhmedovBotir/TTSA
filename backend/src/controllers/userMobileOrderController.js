...
const createOrder = async (req, res) => {
    try {
        ...
        // Location ma'lumotlarini tekshirish va foydalanuvchi manzilini yangilash
        if (location) {
            const { regionId, districtId, mfyId } = location;
        ...
        // Manzil ma'lumotlarini tekshirish
        if (deliveryAddress.fullName.trim().length < 2) {
            return res.status(400).json({
                success: false,
                message: "To'liq ism kamida 2 ta belgidan iborat bo'lishi kerak"
            });
        }
        
        if (deliveryAddress.phone.trim().length < 9) {
            return res.status(400).json({
                success: false,
        ...
        if (deliveryAddress.address.trim().length < 10) {
            return res.status(400).json({
                success: false,
        ...
        const order = new Order({
            ...
            location: location ? {
                region: location.regionId || null,
                district: location.districtId || null,
                mfy: location.mfyId || null
            } : null
        });
