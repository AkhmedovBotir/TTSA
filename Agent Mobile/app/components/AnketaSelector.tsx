...
interface AnketaSelectorProps {
  visible: boolean;
  onClose: () => void;
  onConfirm: (customerData: CustomerData, installmentDuration: number, installmentStartDate: Date) => void;
  loading?: boolean;
  totalSum?: number;
}

// INSTALLMENT_DURATIONS will be populated from API
...
export default function AnketaSelector({ 
  ...
  // Modal ochilganda oldingi ma'lumotlarni tozalaymiz va interest rates API ni chaqiramiz
  React.useEffect(() => {
    if (visible) {
      console.log('=== ANKETA MODAL OPENED ===');
      console.log('Fetching interest rates for installment payment...');
      ...
      // Interest rates API ni chaqiramiz
      fetchInterestRates();
    }
  }, [visible]);
  ...
  const [availableDurations, setAvailableDurations] = useState<number[]>([2, 3, 4, 5, 6, 10, 12]);

  // Interest rates API call
  const fetchInterestRates = async () => {
    try {
      console.log('=== FETCHING INTEREST RATES ===');
      console.log('API URL:', getApiUrl(API_CONFIG.ENDPOINTS.INTEREST_RATES));
      
      const token = await getValidToken();
      if (!token) {
        console.log('No valid token found for interest rates API');
        return;
      }

      console.log('Token found, making request...');

      const response = await fetch(getApiUrl(API_CONFIG.ENDPOINTS.INTEREST_RATES), {
        method: 'GET',
        headers: {
      ...
      console.log('=== INTEREST RATES API RESPONSE ===');
      console.log('Status:', response.status);
      console.log('Response data:', data);
      console.log('=====================================');

      if (response.ok && data.success) {
        console.log('Interest rates loaded successfully:', data.data);
        const rates = data.interestRates || [];
        setInterestRates(rates);
        
        // Update available durations from API data
        const durations = rates
          .filter((rate: InterestRate) => rate.isActive)
          .map((rate: InterestRate) => rate.duration)
          .sort((a: number, b: number) => a - b);
        
        setAvailableDurations(durations);
        console.log('Available durations updated:', durations);
      } else {
        console.log('Failed to load interest rates:', data.message || 'Unknown error');
      }
    } catch (error) {
  ...
  const handleConfirm = () => {
    ...
    if (validateForm()) {
      ...
      onConfirm(customerData, selectedDuration, installmentStartDate);
    } else {
  ...
  const calculateMonthlyPayment = () => {
    ...
    // Find interest rate for selected duration
    const interestRate = interestRates.find(rate => 
      rate.duration === selectedDuration && rate.isActive
    );
    
    if (!interestRate) {
      // Fallback to no interest if rate not found
      return Math.ceil(sum / selectedDuration);
    }
    
    // Calculate with interest rate
    const interestAmount = (sum * interestRate.interestRate) / 100;
    const totalWithInterest = sum + interestAmount;
    
    console.log('=== MONTHLY PAYMENT CALCULATION ===');
    console.log('Original sum:', sum);
    console.log('Selected duration:', selectedDuration);
    console.log('Interest rate:', interestRate.interestRate + '%');
    console.log('Interest amount:', interestAmount);
    console.log('Total with interest:', totalWithInterest);
    console.log('Monthly payment:', Math.ceil(totalWithInterest / selectedDuration));
    console.log('=====================================');
    
    return Math.ceil(totalWithInterest / selectedDuration);
  };
      ...
      >
        ...
        >
          ...
          >
            ...
            <View style={styles.section}>
              ...
              <View style={styles.durationContainer}>
                {availableDurations.map((duration) => {
                  const interestRate = interestRates.find(rate => 
                    rate.duration === duration && rate.isActive
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      {interestRate && (
                        <Text style={[
                          styles.interestRateText,
                          selectedDuration === duration && styles.interestRateTextSelected
                        ]}>
                          {interestRate.interestRate}% foiz
                        </Text>
                      )}
                    </TouchableOpacity>
                  );
                ...
                {totalSum > 0 ? (
                  <>
                    ...
                    {(() => {
                      ...
                      const interestAmount = interestRate ? (totalSum * interestRate.interestRate) / 100 : 0;
                      
                      return (
                        <>
                          <Text style={styles.paymentInfoText}>
                            Foiz: {interestRate ? interestRate.interestRate + '%' : '0%'}
                            {!interestRate && ' (foizsiz)'}
                          </Text>
                          {interestRate && interestAmount > 0 && (
                            <Text style={styles.paymentInfoText}>
                              Foiz miqdori: {interestAmount.toLocaleString()} so'm
                            </Text>
                          )}
                        </>
                      );
            ...
            {/* Muddatli to'lov boshlanish sanasi */}
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>Muddatli to'lov boshlanish sanasi</Text>
              <TouchableOpacity
              ...
              >
                ...
                <Text style={styles.dateInputText}>
                  {installmentStartDate.toLocaleDateString('uz-UZ', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                  })}
              ...
              {showInstallmentDatePicker && (
                <DateTimePicker
                  ...
                  onChange={(event, selectedDate) => {
                    ...
                    if (selectedDate) {
                      setInstallmentStartDate(selectedDate);
                    }
                  }}
        ...
        <View style={styles.container}>
          ...
          >
            <View style={styles.scheduleSummary}>
              ...
              <Text style={styles.scheduleSummaryText}>Boshlang'ich summa: {(totalSum || 0).toLocaleString()} so'm</Text>
              <Text style={styles.scheduleSummaryText}>Umumiy to'lov: {totalPayment.toLocaleString()} so'm</Text>
